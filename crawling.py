        ...
        if res.status_code == 200:
            for index, info in enumerate(matchinfo):
                ...
                if matches["info"]["gameType"] == "MATCHED_GAME" and matches["info"]["gameMode"] == "ARAM" or matches["info"]["gameMode"] == "CLASSIC":
                    detail["game_duration"] = matches["info"]["gameDuration"]
                    detail["game_timestamp"] = matches["info"]["gameEndTimestamp"]
                    detail["game_mode"] = info
                    ...
                    for cnt in range(0, count):
                        if matches["info"]["participants"][cnt]["summonerName"] == name:
                            ...
                            detail["spell_fir"] = matches["info"]["participants"][cnt]["summoner1Id"]
                            detail["spell_sec"] = matches["info"]["participants"][cnt]["summoner2Id"]
