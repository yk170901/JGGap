...
def lol_record(name):
    ...
    if res.status_code == 200:
        ...
        if res.status_code == 200:
            for index, info in enumerate(matchinfo):
                detail = {}
                participant = {}
                ...
                if matches["info"]["gameType"] == "MATCHED_GAME" and matches["info"]["gameMode"] == "ARAM" or matches["info"]["gameMode"] == "CLASSIC":
                    ...
                    for cnt in range(0, 10):
                        if matches["info"]["participants"][cnt]["summonerName"] == name:
                            ...
                            if matches["info"]["participants"][cnt]["championName"] == "FiddleSticks":
                                detail["champion_name"] = "Fiddlesticks"
                            else:
                        ...
                        for cnt in range(0, 10):
                            if cnt < 5:
                                ...
                                if matches["info"]["participants"][cnt]["championName"] == "FiddleSticks":
                                    participant["blue_champion" + str(cnt + 1)] = "Fiddlesticks"
                                else:
                                ...
                                if matches["info"]["participants"][cnt]["championName"] == "FiddleSticks":
                                    participant["red_champion" + str(cnt-4)] = "Fiddlesticks"
                                else:
...
def insert_lol_info(name, user_no):
    api_key = "RGAPI-f2e8aba1-e75d-47c8-8829-e097207ecd61"
    ...
    if res.status_code == 200:
        ...
        if user_no == "0":
            user_no = "NULL"
        sql = "insert into lol_info values (LOL_INFO_NO.NEXTVAL, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"%(user_no, "'"+name+"'", "'"+lol_info["solo_tier"]+"'", lol_info["solo_tier_grade"], "'"+lol_info["free_tier"]+"'", lol_info["free_tier_grade"], str(lol_info["solo_win"]), str(lol_info["solo_lose"]), str(lol_info["solo_lp"]), str(lol_info["free_win"]), str(lol_info["free_lose"]), str(lol_info["free_lp"]))

        cursor.execute(sql)
        cursor.close()
        conn.close()
        print("insert_lol_info")
...
if __name__ == "__main__":
    insert_lol_info(sys.argv[1], sys.argv[2])
    lol_record(sys.argv[1])
